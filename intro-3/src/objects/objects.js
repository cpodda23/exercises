...
export function arrayToObjectDeep(array) {
  ...
  return Array.isArray(arrToObj) ? arrayToObjectDeep(arrToObj) : arrToObj
}
...
export function getTreeDepth(tree) {
  return 1 + Math.max(0, ...(tree.children || []).map((item) => getTreeDepth(item)))
}
...
export function countTreeLeafNodes(tree) {
  return (
    1 + Math.max(0, ...(tree.children || []).reduce((acc, item) => (acc += item.children ? 1 + getTreeDepth(item) : 1))))
}
